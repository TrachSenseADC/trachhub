...
def create_cleaning():
    ...
    with Session(engine) as session:
        cleaning = Cleaning(
            note=data["note"], timestamp=datetime.fromisoformat(data["timestamp"])
        )
...
def get_events_last_24h():
    ...
    with Session(engine) as session:
        ...
        cleanings = (
            session.query(Cleaning).filter(Cleaning.timestamp >= yesterday).all()
        )
...
def get_events_this_month():
    ...
    with Session(engine) as session:
        anomalies = (
            session.query(Anomaly).filter(Anomaly.start_time >= start_of_month).all()
        )
        notes = session.query(Note).filter(Note.timestamp >= start_of_month).all()
        cleanings = (
            session.query(Cleaning).filter(Cleaning.timestamp >= start_of_month).all()
        )
